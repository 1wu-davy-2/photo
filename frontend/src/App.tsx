...
import type { Folder, Photo, SortOrder, UploadQueueState } from "./types/photo";
...
function GalleryApp({ session, locale, t, route, onLocaleChange, onNavigate, onLogout, theme, onThemeChange, pendingDrop, onPendingDropHandled }: { session: AuthSession; locale: Locale; t: Translator; route: AppRoute; onLocaleChange: (locale: Locale) => void; onNavigate: (route: AppRoute) => void; onLogout: () => void; theme: ColorTheme; onThemeChange: (theme: ColorTheme) => void; pendingDrop: DataTransfer | null; onPendingDropHandled: () => void }) {
  ...
  const [uploadConfirmation, setUploadConfirmation] = useState<{ folderName?: string; files: File[] } | null>(null);
  const [selectedPhoto, setSelectedPhoto] = useState<Photo | null>(null);
  const uploadOpenRef = useRef<(() => void) | null>(null);
  const uploadQueueRef = useRef<UploadQueueRuntime | null>(null);
  ...
  const processUploadQueue = useCallback(async () => {
    const runtime = uploadQueueRef.current;
    if (!runtime || uploadRunningRef.current) return;
    uploadRunningRef.current = true;
    try {
      while (uploadQueueRef.current === runtime && runtime.nextIndex < runtime.files.length && !runtime.paused) {
        const file = runtime.files[runtime.nextIndex];
        setUploadQueue((current) => current ? { ...current, currentName: file.name, currentProgress: 0, isPaused: false } : current);
        try {
          await uploadPhoto(file, (progress) => setUploadQueue((current) => current ? { ...current, currentProgress: progress } : current), session.accessToken, runtime.folderId);
        } catch (uploadError) {
          runtime.failedFiles.push(file);
          setError(uploadError instanceof Error ? uploadError.message : t("manage.error"));
        }
        runtime.nextIndex += 1;
        setUploadQueue((current) => current ? { ...current, completed: runtime.nextIndex, currentName: runtime.nextIndex < runtime.files.length ? runtime.files[runtime.nextIndex].name : "", currentProgress: 0, isPaused: runtime.paused, failed: runtime.failedFiles.length } : current);
      }

      if (runtime.nextIndex >= runtime.files.length && uploadQueueRef.current === runtime) {
        setUploadQueue((current) => current ? { ...current, currentName: "", currentProgress: 100, isPaused: false, failed: runtime.failedFiles.length } : current);
        setNotice(`${runtime.files.length} ${t("gallery.uploadSuccess")}`);
        try {
          await refresh();
          setFolders(await listFolders("owned", session.accessToken));
        } catch (refreshError) {
          setError(refreshError instanceof Error ? refreshError.message : t("manage.error"));
        }
      }
    } finally {
      uploadRunningRef.current = false;
    }
  }, [refresh, session.accessToken, t]);

  const startUploadQueue = useCallback((files: File[], folderId?: string) => {
    const images = files.filter((candidate) => candidate.type.startsWith("image/"));
    if (images.length === 0) { setError(t("gallery.uploadSupport")); return; }
    const runtime: UploadQueueRuntime = { files: images, folderId, nextIndex: 0, paused: false, failedFiles: [] };
    uploadQueueRef.current = runtime;
    setError(""); setNotice("");
    setUploadQueue({ total: images.length, completed: 0, currentName: images[0].name, currentProgress: 0, isPaused: false, failed: 0 });
    void processUploadQueue();
  }, [processUploadQueue, t]);

  const requestUploadConfirmation = (files: FileList | File[], folderName?: string) => {
    const images = Array.from(files).filter((candidate) => candidate.type.startsWith("image/"));
    if (images.length === 0) { setError(t("gallery.uploadSupport")); return; }
    setError(""); setNotice("");
    setUploadConfirmation({ files: images, folderName });
  };

  const handleFiles = (files: FileList | File[]) => {
    requestUploadConfirmation(files);
  };

  const handleDataTransfer = async (dataTransfer: DataTransfer) => {
    try {
      const dropped = await collectDroppedUpload(dataTransfer);
      requestUploadConfirmation(dropped.files, dropped.folderName);
    } catch (dropError) {
      setError(dropError instanceof Error ? dropError.message : t("manage.error"));
    }
  };
  ...
  const confirmFolderUpload = async (parentId: string, shouldCreate: boolean) => {
    ...
    startUploadQueue(uploadDialog.files, targetFolder.id);
    setUploadDialog(null);
  };

  const confirmUpload = async () => {
    if (!uploadConfirmation) return;
    const pending = uploadConfirmation;
    if (!pending.folderName) {
      setUploadConfirmation(null);
      startUploadQueue(pending.files, defaultFolderId);
      return;
    }
    try {
      let nextFolders = folders;
      if (nextFolders.length === 0) {
        nextFolders = await listFolders("owned", session.accessToken);
        setFolders(nextFolders);
      }
      if (nextFolders.length === 0) {
        setError(t("manage.noFolders"));
        return;
      }
      setUploadConfirmation(null);
      setUploadDialog({ folderName: pending.folderName, files: pending.files });
    } catch (folderError) {
      setError(folderError instanceof Error ? folderError.message : t("manage.error"));
    }
  };

  const pauseUpload = () => {
    if (!uploadQueueRef.current) return;
    uploadQueueRef.current.paused = true;
    setUploadQueue((current) => current ? { ...current, isPaused: true } : current);
  };

  const resumeUpload = () => {
    if (!uploadQueueRef.current) return;
    uploadQueueRef.current.paused = false;
    setUploadQueue((current) => current ? { ...current, isPaused: false } : current);
    void processUploadQueue();
  };
  ...
  const retryFailedUploads = () => {
    const runtime = uploadQueueRef.current;
    if (!runtime || runtime.failedFiles.length === 0) return;
    startUploadQueue(runtime.failedFiles, runtime.folderId);
  };
  ...
  return (
    <AppShell total={total} username={session.user.username} role={session.user.role} locale={locale} route={route} t={t} onLocaleChange={onLocaleChange} onNavigate={onNavigate} onUpload={() => uploadOpenRef.current?.()} onLogout={onLogout} theme={theme} onThemeChange={onThemeChange} uploadQueue={uploadQueue} onPauseUpload={pauseUpload} onResumeUpload={resumeUpload} onDismissUpload={dismissUpload} onRetryUpload={retryFailedUploads}>
      ...
      <UploadDropzone upload={null} onFiles={handleFiles} onDataTransfer={handleDataTransfer} onChooseRef={(open) => { uploadOpenRef.current = open; }} t={t} />
      ...
      {uploadConfirmation && <UploadConfirmDialog fileCount={uploadConfirmation.files.length} folderName={uploadConfirmation.folderName} t={t} onCancel={() => setUploadConfirmation(null)} onConfirm={confirmUpload} />}
